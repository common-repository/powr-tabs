<?php
     ...
     */
    /*
    Plugin Name: POWr Tabs
    Plugin URI: https://www.powr.io/plugins/tabs
    Description: Help visitors navigate your site and content  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create Tabs widget/////////////////////////////////
    class Powr_Tabs extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_tabs',
                             __( 'POWr Tabs' ),
                             array( 'description' => __( 'Tabs by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-tabs' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Tabs.
        </p>
        <p>
    ...
    function register_powr_tabs() {
      register_widget( 'Powr_Tabs' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_tabs' );
    //Create short codes for adding plugins anywhere:
    function powr_tabs_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-tabs' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-tabs' label='$label'></div>";
      }else{
      	"<div class='powr-tabs'></div>";
      }
    }
    add_shortcode( 'powr-tabs', 'powr_tabs_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_tabs_menu' );
    function powr_tabs_menu() {
      add_menu_page( 'POWr Tabs', 'POWr Tabs', 'manage_options', 'powr-tabs-settings', 'powr_tabs_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_tabs_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/tabs';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/tabs';
      }
    ...
    register_activation_hook(__FILE__, 'powr_tabs_plugin_activate');
      add_action('admin_init', 'powr_tabs_plugin_redirect');

      function powr_tabs_plugin_activate() {
      add_option('powr_tabs_plugin_do_activation_redirect', true);
      }
      ...
      function powr_tabs_plugin_redirect() {
      if (get_option('powr_tabs_plugin_do_activation_redirect', false)) {
          delete_option('powr_tabs_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-tabs-settings&' );
          }
       }
      }
